...
function renderPr_Po_Sp(JSONPr,Po_Sp){
    ...
    if(Po_Sp.length === 0){
      for(var key in JSONPr){
        if(key==="Po" || key==="id" || key==="purchase_ids" || key==="picking_ids")
          continue;
...
function test(){
   tabPr = custom_oe_browse("purchase.requisition","company_id name user_id ordering_date date_end schedule_date origin state description purchase_ids","[['state','not in',['draft','cancel']],"+filterDate()+"]");
   tabPo = custom_oe_browse("purchase.order","requisition_id partner_ref date_approve partner_id date_planned user_id origin amount_untaxed amount_total currency_id state notes id picking_ids","[]");
   tabSp = custom_oe_browse("stock.picking","purchase_id name partner_id user_id scheduled_date origin backorder_id state priority picking_type_id","[]")
...
function getData(){
    tabPr = custom_oe_browse("purchase.requisition","company_id name user_id ordering_date date_end schedule_date origin state description purchase_ids","[['state','not in',['draft','cancel']],"+filterDate()+"]");
    tabPo = custom_oe_browse("purchase.order","requisition_id partner_ref date_approve partner_id date_planned user_id origin amount_untaxed amount_total currency_id state picking_ids","[]");
